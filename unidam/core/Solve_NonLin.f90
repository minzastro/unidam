! Taken from https://jblevins.org/mirror/amiller/hbrd.f90
MODULE Solve_NonLin
...
REAL  :: xtol
REAL, PARAMETER  :: factor = 100.0, zero = 0.0
...
INTEGER    :: iwa(1)
LOGICAL    :: jeval, sing
REAL  :: actred, delta, epsmch, fnorm, fnorm1, pnorm, prered,   &
              ratio, sum, temp, xnorm
REAL, PARAMETER  :: one = 1.0, p1 = 0.1, p5 = 0.5,   &
                         p001 = 0.001, p0001 = 0.0001, zero = 0.0

! The following were workspace arguments
REAL  :: fjac(n,n), r(n*(n+1)/2), qtf(n), wa1(n), wa2(n),   &
              wa3(n), wa4(n)
...
REAL, INTENT(OUT)  :: q(ldq,m)
REAL, INTENT(OUT)  :: wa(m)
...
REAL, INTENT(OUT)     :: rdiag(n)
REAL, INTENT(OUT)     :: acnorm(n)
...
REAL, PARAMETER  :: one = 1.0, p05 = 0.05, zero = 0.0
...
REAL, PARAMETER  :: one = 1.0, p5 = 0.5, p25 = 0.25, zero = 0.0
...
REAL, PARAMETER  :: rdwarf = tiny(0.0), rgiant = huge(1.0)
...
!              SUM FOR SMALL COMPONENTS.
      ...
      IF (xabs > x3max) THEN
        ...
        x3max = xabs
      ELSE
        IF (xabs /= 0.0) s3 = s3 + (xabs/x3max) ** 2
      END IF
    END IF
  ELSE
...
ELSE
  IF (s2 /= 0.0) THEN
    IF (s2 >= x3max) fn_val = SQRT(s2*(1.0 + (x3max/s2)*(x3max*s3)))
